...
import org.apache.cordova.radiocoin.wallet.util.Qr;
    ...
    public boolean onClickAddressContextMenuItem(final MenuItem item, final Address address, final String label) {
        ...
        if (itemId == R.id.wallet_addresses_context_edit) {
            ...
            if (address instanceof LegacyAddress || ownName != null)
                uri = BitcoinURI.convertToBitcoinURI(address, null, ownName, null).replace(AbstractBitcoinNetParams.BITCOIN_SCHEME, "radiocoin-insert-here");
            else
                uri = address.toString().toUpperCase(Locale.US).replace(AbstractBitcoinNetParams.BITCOIN_SCHEME, "radiocoin-insert-here");
