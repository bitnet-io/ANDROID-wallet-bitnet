...
import org.apache.cordova.radiocoin.wallet.data.AbstractWalletLiveData;
import org.apache.cordova.radiocoin.wallet.data.ConfigOwnNameLiveData;
...
public class WalletAddressViewModel extends AndroidViewModel {
    ...
    private String uri(final Address address, @Nullable final String label) {
        ...
        if (address instanceof LegacyAddress || label != null)
            return BitcoinURI.convertToBitcoinURI(address, null, label, null).replace(AbstractBitcoinNetParams.BITCOIN_SCHEME, "");
        else
            return address.toString().toUpperCase(Locale.US).replace(AbstractBitcoinNetParams.BITCOIN_SCHEME, "");
    }
